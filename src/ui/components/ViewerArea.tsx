import { h, VNode, RefObject } from "preact";
      ...
      {mode === "split" && !splitSourceBitmap ? (
         <div className="viewer-empty-state">
            <div className="viewer-upload-box">
               <div style={{ marginBottom: "1rem", color: "var(--color-text-muted)", fontSize: "0.85rem", maxWidth: "100%", margin: "0 auto 1rem", lineHeight: "1.4" }}>{t("selectImageTip")}</div>
               <label className="btn btn-primary" style={{ display: "inline-flex", flexDirection: "row", alignItems: "center", gap: "8px", padding: "0.6rem 1.25rem", cursor: "pointer", width: "auto", maxWidth: "100%", whiteSpace: "nowrap" }}>
         ...
         <div ref={containerRef} className="viewer-canvas checkerboard-bg" style={{ cursor: isPanning ? "grabbing" : "grab" }}
           ...
           <div className="floating-badge-container">
              {mode === 'stitch' && <div className="floating-badge badge-primary">@{task.artistHandle} / x.com/{task.tweetId}</div>}
              <div className="floating-badge">{mode === 'split' ? (splitSourceBitmap ? `${splitSourceBitmap.width} × ${splitSourceBitmap.height} px` : "") : (canvasSize.width > 0 ? `${canvasSize.width} × ${canvasSize.height} px` : "")}</div>
           </div>
           ...
           {isGenerating || isSplitting ? (
             <div className="loading-overlay">
                ...
                <div className="loading-text">{mode === "stitch" && isGenerating ? t("stitching") : t("processing")}</div>
             </div>
           ) : null}
           ...
           <div className="viewer-toolbar-v2 apple-blur" style={{ position: "absolute", bottom: "1.5rem", left: "50%", transform: "translateX(-50%)", zIndex: 30 }}>
               ...
               <div className="toolbar-row secondary">
                   ...
                   {mode === 'split' && splitSourceBitmap && onClearSplit && (
                       <>
                           <div className="toolbar-divider-v" />
                           <IconButton className="toolbar-btn" onClick={onClearSplit} icon={<Trash2 size={16} />} style={{ background: "none", border: "none", color: "#ff453a" }} title={t("removeImage")} />
                       </>
                   )}
               </div>
           </div>
           ...
           <div style={{ transform: `translate(${viewerOffset.x}px, ${viewerOffset.y}px) scale(${viewerScale}) rotate(${viewerRotation}deg)`, transition: isPanning ? "none" : "transform 0.2s cubic-bezier(0.2, 0, 0, 1)", transformOrigin: "center center" }}>
               {mode === "split" ? (
                  ...
                  previewUrl ? <img src={previewUrl} className="viewer-image" style={{ width: `${canvasSize.width}px`, height: `${canvasSize.height}px` }} /> : null
               )}
           </div>
         </div>
      )}
