...
export function App({ task, onClose }: AppProps) {
  ...
  useEffect(() => {
    localStorage.setItem('x-puzzle-stitcher-format', outputFormat);
  }, [outputFormat]);
  ...
  const [backgroundColor, setBackgroundColor] = useState<BackgroundColor>(() => {
    const saved = localStorage.getItem('x-puzzle-stitcher-bg');
    return (saved as any) || 'transparent';
  });
  ...
  useEffect(() => {
    localStorage.setItem('x-puzzle-stitcher-bg', backgroundColor);
  }, [backgroundColor]);
  ...
  const handleDoubleClick = () => {
    if (viewerScale < 1) resetViewer(false); // 从缩放模式跳到 1:1
  ...
  const download = async () => {
    ...
    try {
      ...
      const safeTitle = task.pageTitle.replace(/[\\/:*?"<>|]/g, '_').substring(0, 50);
  ...
  return (
    <div className="modal-overlay" style={{
      ...
      <div className="card" style={{
        width: '860px', height: '85vh', display: 'flex', flexDirection: 'column',
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          >
            {isGenerating && (
              <div style={{ 
                ...
                gap: '1rem', color: 'white', transition: 'all 0.3s',
                pointerEvents: 'none'
              }}>
          ...
          <div style={{ 
            width: '260px', borderLeft: '1px solid var(--color-border)', 
                    ...
                    {['PNG', 'JPG', 'WEBP'].map(fmt => (
                      <button 
                        ...
                        onClick={() => {
                          ...
                          if (newFmt === 'jpg' && backgroundColor === 'transparent') {
                            setBackgroundColor('white');
                          }
                        }}
                        style={{ 
                          flex: 1, padding: '6px', border: 'none', borderRadius: 'var(--radius-sm)',
                          backgroundColor: outputFormat === fmt.toLowerCase() ? 'var(--color-primary)' : 'transparent',
                          color: outputFormat === fmt.toLowerCase() ? 'white' : 'var(--color-text)',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                   ...
                   <div style={{ display: 'flex', backgroundColor: 'white', borderRadius: 'var(--radius-md)', padding: '3px', border: '1px solid var(--color-border)', gap: '2px', flex: 1 }}>
                    {(['transparent', 'white', 'black'] as const).map(bg => (
                      <button 
                        key={bg}
                        disabled={outputFormat === 'jpg' && bg === 'transparent'}
                        onClick={() => setBackgroundColor(bg)}
                        style={{ 
                          flex: 1, padding: '4px', border: 'none', borderRadius: 'var(--radius-sm)',
                          backgroundColor: backgroundColor === bg ? 'var(--color-secondary)' : 'transparent',
                          color: backgroundColor === bg ? 'white' : 'var(--color-text)',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', backgroundColor: 'white', border: '1px solid var(--color-border)', borderRadius: '4px', overflow: 'hidden' }}>
                      <button 
                        onClick={() => setGlobalGap(Math.max(-20, globalGap - 1))}
                        style={{ padding: '2px 4px', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', borderRight: '1px solid var(--color-border)' }}
                      ...
                      <input 
                        ...
                        style={{ 
                          width: '32px', height: '22px', fontSize: '11px', border: 'none',
                          outline: 'none', textAlign: 'center', backgroundColor: 'transparent',
                          fontWeight: 'bold', color: 'var(--color-primary)', display: 'block'
                        }}
                      />
                      <button 
                        onClick={() => setGlobalGap(Math.min(100, globalGap + 1))}
                        style={{ padding: '2px 4px', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', borderLeft: '1px solid var(--color-border)' }}
                      ><Plus size={12}/></button>
                    </div>
                    <button 
                    ...
                    ><RotateCcw size={12} color="var(--color-text-muted)"/></button>
                    <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginLeft: '2px' }}>px</span>
                  </div>
                </div>
                <input 
                  type="range" min="-20" max="100" value={globalGap} 
                  onChange={(e) => setGlobalGap(parseInt((e.target as HTMLInputElement).value))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {images.map((img, idx) => (
                    <div 
                      ...
                      onDragEnd={onDragEnd}
                      style={{ 
                        display: 'flex', flexDirection: 'column', gap: '0.4rem', padding: '0.625rem', 
                        backgroundColor: 'white', border: '1px solid #F1F5F9', // 使用极浅边框
                        borderRadius: 'var(--radius-md)', 
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        ...
                        <div 
                          title="按住拖拽排序"
                          style={{ cursor: 'grab', color: '#94A3B8', display: 'flex', alignItems: 'center', opacity: 0.6 }}
                        >
                        ...
                        <div style={{ position: 'relative', flexShrink: 0 }}>
                          <img src={img.thumbnailUrl} style={{ width: '42px', height: '42px', objectFit: 'cover', borderRadius: 'var(--radius-sm)', border: '1px solid #F1F5F9' }} />
                          <div style={{ 
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          ...
                          <button 
                            ...
                            style={{ 
                              ...
                              color: img.visible === false ? '#EF4444' : '#64748B',
                              opacity: img.visible === false ? 0.8 : 0.6
                            }}
                          >
                            {img.visible !== false ? <Eye size={16}/> : <EyeOff size={16}/>}
                          </button>
                        </div>
                      </div>
            ...
            <div style={{ padding: '1rem', borderTop: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
              <button 
                ...
                style={{ width: '100%', height: '3rem', fontSize: '0.875rem', boxShadow: '0 4px 12px rgba(249, 115, 22, 0.3)' }}
              >
                {isGenerating ? (
                   <>
...
function LayoutButton({ active, onClick, icon, label }: any) {
  return (
    <button onClick={onClick} style={{
      ...
      backgroundColor: active ? 'rgba(59, 130, 246, 0.05)' : 'var(--color-surface)', 
      cursor: 'pointer', color: active ? 'var(--color-primary)' : 'var(--color-text)',
      transition: 'all var(--transition-fast)',
