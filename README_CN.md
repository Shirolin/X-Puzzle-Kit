...
### ✂️ 智能拆图 (Smart Splitter)

- **多模式切分**: 提供 **2x2 网格**、**T型（1大2小）**、**自定义行列 (Nx1, 1xN)** 等多种布局，完美适配 Twitter 的四图流或长条漫展示。
- **精细化编辑**: 支持 **整体 (Unified)** 与 **分别 (Individual)** 两种拖拽模式。既可统筹全局缩放移动，也能对单个格子进行独立调整，甚至**替换单格图片**。
- **推特优化**: 内置一键 **“推特比例优化”**，自动调整裁切比例以确保在 Timeline 上完美展示。
- **个性化输出**: 支持调整 **间距 (Gap)**、**填充背景色 (Background Fill)**，并可选择导出为 **ZIP 包** 或单张图片。
