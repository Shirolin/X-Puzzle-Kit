...
本稳当基于《产品需求文档 (PRD) v1.0》编写，详细阐述 X-Puzzle-Kit (X (Twitter) 拼图还原助手) 的技术实现方案。
...
| 模块                            | 职责描述                                                            | 技术选型                                |
| ------------------------------- | ------------------------------------------------------------------- | --------------------------------------- |
| **Content Script**              | 注入 X (Twitter) 页面，负责 DOM 监听、图片 URL 提取、挂载 UI 容器。 | TypeScript, Native DOM API              |
...
interface SplitConfig {
  layout: "GRID_2x2" | "VERTICAL_1xN" | "HORIZONTAL_Nx1" | "T_SHAPE_3";
  rows: number; // For Nx1/1xN
  cols: number; // For Nx1/1xN
  gap: number; // 消除间隙 (px)
  format: "png" | "jpg" | "webp";
  autoCropRatio?: number; // Twitter 优化比例 (如 16/9)
}
```
...
### 3.3 图片分割算法 (Image Splitting) - F6 实现

该算法与拼接互逆，用于将单张大图切割为多张小图。

1.  **网格计算**: 根据 `SplitConfig.layout` 计算切割网格 (Grid Matrix)。
    - Example `GRID_2x2`: 切分为 4 个象限 `(0,0, w/2, h/2), (w/2,0, w/2, h/2) ...`
2.  **间隙移除 (Gap Removal)**:
    - 若 `gap > 0`，则在切割时，**跳过**中间区域的像素。
    - 例如水平切割，第一张取 `0 ~ (h/2 - gap/2)`，第二张取 `(h/2 + gap/2) ~ h`。
3.  **X (Twitter) 比例优化 (Auto Crop)**:
    - 若启用 `isXTwitterOptimized`，则强制将每个切片的目标 Canvas 宽高比设为 16:9。
    - 使用 `object-fit: cover` 逻辑：保持短边完整，裁剪长边（居中裁剪）。
4.  **导出**: 使用 `canvas.toBlob` 生成 `Blob[]` 数组。
...
### 4.2 右键菜单通信 (Context Menu Flow) - F7.1 实现
...
sequenceDiagram
    ...
    User->>BG: Right Click Image -> "Split Image"
    BG->>BG: contextMenus.onClicked
    BG->>CS: chrome.tabs.sendMessage (OPEN_SPLITTER, srcUrl)
    CS->>CS: 解析消息
    CS->>UI: mountUI(mode='split', url)
    activate UI
    UI->>BG: FETCH_IMAGE (srcUrl) [如果跨域]
    BG-->>UI: DataURL / Blob
    UI->>UI: Create ImageBitmap & Render
    deactivate UI
```

---

## 5. 数据存储 (Storage & Persistence)

使用 `chrome.storage.local` 保存用户偏好设置。

| Key                          | Type   | Description                                 |
| ---------------------------- | ------ | ------------------------------------------- |
| `x-puzzle-kit-lang`          | string | 语言设置 (auto/zh_CN/en/...)                |
| `x-puzzle-kit-theme`         | string | 主题设置 (auto/light/dark)                  |
| `x-puzzle-kit-format`        | string | 输出格式 (png/jpg)                          |
| `x-puzzle-kit-bg`            | string | 背景色 (transparent/white/black)            |
| `x-puzzle-kit-split-options` | object | 分割器配置对象 (format, isZip, isOptimized) |
...
/src
  ...
  /ui            # React/Preact UI
    ...
    components/
      Sidebar.tsx        # 侧边栏控制器
      ViewerArea.tsx     # 预览与画布区域
      SplitterControl.tsx # 分割器控制面板
      Common.tsx         # 通用组件
  /core          # 核心算法 (无 DOM/UI 依赖)
    stitcher.ts  # 拼图 Canvas 绘制逻辑
    splitter.ts  # 分割 Canvas 处理逻辑
    layout.ts    # 布局推荐
    i18n.ts      # 国际化逻辑
  /manifest.json
```
