...
| :----------- | :------------------------------------------------ | :----------------------------------------------- |
| **主要目标** | 深度集成于 X (Twitter) 网页，提供沉浸式操作体验。 | 提供跨平台的独立工具，无需安装插件，支持移动端。 |
...
| **URL 导入解析**     | **不支持**。严格禁止任何后端 API 访问及 URL 导入功能。 | **核心功能**。通过后端 API (Cloudflare Workers) 抓取推文元数据。 |
...
- **插件端** 优势在于**效率**。用户在浏览 X (Twitter) 时可以“即看即拆”，流程极短。
...
2. **严格合规**：确保 Chrome 插件端所有代码逻辑运行在本地，不触碰任何远程业务 API，满足商店审核的隐私与安全要求。
